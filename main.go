...
func getCommands() map[string]cliCommand {
	return map[string]cliCommand{
		...
		"inspect": {
			name:        "inspect",
			description: "Shows details about a Pokemon you've caught: inspect <pokemon_name>",
...
func commandInspect(cfg *config, args []string) error {
	if len(args) != 1 {
		return fmt.Errorf("inspect requires exactly one argument: the pokemon name")
	}
	name := args[0]

	// No API call: inspect only works on Pokemon already stored by catch.
